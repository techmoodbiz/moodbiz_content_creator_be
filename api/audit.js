...
export default async function handler(req, res) {
  ...
  const token = parts[1]?.trim(); // Fixed usage of parts undefined
  const parts = authHeader.split('Bearer '); // Fixed definition order

  try { await admin.auth().verifyIdToken(parts[1]); }
...
`;
    ...
    if (!parsedResult) {
      parsedResult = {
        ...
        identified_issues: [{ category: "ai_logic", severity: "Low", problematic_text: "System Error", citation: "System", reason: "Invalid JSON", suggestion: "Thử lại." }]
      };
    }
